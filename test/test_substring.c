...
void test_getSubstring(void)
{
	...
	char expected_string[26][MAX_FIELD] = {	{"2019-09-01"}, {"2019-09-02"},
										...
										{"mo"}, {"ads"},
										{"ZONE=Test"}, {"START=15:30"},
										{"END=16:30"}, {"CONTEXT=work"}};
	...
	for(int i = 0 ; i < TESTS ; i++) {
		result_set[i] = getSubstring(	&test[i][0],
					  					string_set[i],
					  				  	&amount_substrings_set[i],
										sep[i]);
